...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public static class MetaData
    {
        ...
        static readonly Type nameAttr = typeof(StaticEcsEditorNameAttribute);
        static readonly Type valueAttr = typeof(StaticEcsEditorTableValueAttribute);
        static readonly Dictionary<Type, FieldInfo[]> _typesCacheWithNonPublic = new();
        ...
        static void Init()
        {
            ...
            foreach (var type in assembly.GetTypes())
            {
                ...
                {
                    if (interfaces.Contains(typeof(IComponent))) componentTypes.Add(type);

                    if (interfaces.Contains(typeof(ITag))) tagTypes.Add(type);

                    if (interfaces.Contains(typeof(IEvent))) eventTypes.Add(type);

                    if (interfaces.Contains(typeof(IEntityType)))
                    {
                        var meta = CreateEntityTypeMeta(type);
                        if (meta != null) entityTypes.Add(meta);
                    }

                    if (interfaces.Contains(typeof(ILinksType)))
                        linksTypes.Add(type);
                    else if (interfaces.Contains(typeof(ILinkType))) linkTypes.Add(type);

                    if (interfaces.Contains(typeof(IMultiComponent))) multiTypes.Add(type);
                }
            }
            ...
            foreach (var worldType in worldTypes)
            {
                ...
                foreach (var type in componentTypes) HandleComponentMeta(data, type);

                foreach (var type in tagTypes) HandleTagMeta(data, type);

                foreach (var type in eventTypes) HandleEventMeta(data, type);
                ...
                {
                    var linksComponentType =
                        worldOpenType.GetNestedType("Links`1").MakeGenericType(worldType, linksType);
                    HandleComponentMeta(data, linksComponentType);
                }

                foreach (var multiType in multiTypes)
                {
                    var multiComponentType =
                        worldOpenType.GetNestedType("Multi`1").MakeGenericType(worldType, multiType);
        ...
        public static void EnrichByWorld(WorldHandle handle)
        {
            ...
            foreach (var compHandle in handle.GetAllComponentsHandles())
            {
                ...
                if (compHandle.IsTag)
                {
                    if (data.Tags.Find(meta => meta.Type == type) == null) HandleTagMeta(data, type);
                }
                else
                {
                    if (data.Components.Find(meta => meta.Type == type) == null) HandleComponentMeta(data, type);
                }
            }
            ...
            foreach (var eventsHandle in handle.GetAllEventsHandles())
            {
                ...
                if (data.Events.Find(meta => meta.Type == type) == null) HandleEventMeta(data, type);
            }

            data.Enriched = true;
        }

        static (string name, string fullName) NameAttribute(Type type)
        {
        ...
        static void HandleComponentMeta(WorldMetaData data, Type type)
        {
            var fullName = "";
            var name = "";
            ...
            if (string.IsNullOrEmpty(fullName)) fullName = type.EditorFullTypeName();

            if (string.IsNullOrEmpty(name)) name = type.EditorTypeName();

            if (data.Components.Find(meta => meta.FullName == fullName) != null) return;

            var (field, property, width) = FindValueAttribute(type);
            width = Math.Max(GUI.skin.label.CalcSize(new GUIContent(name)).x, width);

            data.Components.Add(new EditorEntityDataMeta(type, name, fullName, width,
                new[] { GUILayout.Width(width), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) },
                new[] { GUILayout.Width(width + 68f), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) }, field,
                property));
        }

        static void HandleTagMeta(WorldMetaData data, Type type)
        {
            var fullName = "";
            var name = "";
            var (n, fn) = NameAttribute(type);
            ...
            if (data.Tags.Find(meta => meta.FullName == fullName) != null) return;

            var width = GUI.skin.label.CalcSize(new GUIContent(name)).x;
            data.Tags.Add(new EditorEntityDataMeta(type, name, fullName, width,
                new[] { GUILayout.Width(width), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) },
                new[] { GUILayout.Width(width + 46f), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) }, null,
                null));
        }

        static void HandleEventMeta(WorldMetaData data, Type type)
        {
            var fullName = "";
            var name = "";
            ...
            if (data.Events.Find(meta => meta.FullName == fullName) != null) return;

            var (field, property, width) = FindValueAttribute(type);
            width = Math.Max(GUI.skin.label.CalcSize(new GUIContent(name)).x, width);

            data.Events.Add(new EditorEventDataMeta(type, name, fullName, width,
                new[] { GUILayout.Width(width), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) },
                new[] { GUILayout.Width(width + 70f), GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight) }, field,
                property));
        }

        static EditorEntityTypeMeta CreateEntityTypeMeta(Type type)
        {
            var (n, _) = NameAttribute(type);
        ...
        static (FieldInfo fieldInfo, PropertyInfo propertyInfo, float width) FindValueAttribute(Type type)
        {
            foreach (var field in type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            foreach (var customAttribute in field.GetCustomAttributesData())
                if (customAttribute.AttributeType.Namespace + customAttribute.AttributeType.FullName ==
                    valueAttr.Namespace + valueAttr.FullName)
                {
        ...
        internal static FieldInfo[] GetCachedSerializableType(Type type)
        {
            if (!_typesCache.TryGetValue(type, out var fields))
            {
                if (!Attribute.IsDefined(type, typeof(SerializableAttribute)) &&
                    !typeof(IComponent).IsAssignableFrom(type) && !typeof(IEvent).IsAssignableFrom(type) &&
                    !typeof(ISystem).IsAssignableFrom(type) && !HasShowAttribute(type))
                {
        ...
        static bool HasShowAttribute(FieldInfo field)
        {
            var showType = typeof(StaticEcsEditorShowAttribute);
            foreach (var customAttribute in field.GetCustomAttributesData())
                if (customAttribute.AttributeType.Namespace + customAttribute.AttributeType.FullName ==
                    showType.Namespace + showType.FullName)
                    return true;

            return false;
        }

        static bool HasShowAttribute(Type type)
        {
            var showType = typeof(StaticEcsEditorShowAttribute);
        ...
        static bool HasHideAttribute(FieldInfo field)
        {
            var hideType = typeof(StaticEcsEditorHideAttribute);
            foreach (var customAttribute in field.GetCustomAttributesData())
                if (customAttribute.AttributeType.Namespace + customAttribute.AttributeType.FullName ==
                    hideType.Namespace + hideType.FullName)
                    return true;
    ...
    public class EditorEventDataMeta
    {
        ...
        public EditorEventDataMeta(Type type, string name, string fullName, float width, GUILayoutOption[] layout,
            GUILayoutOption[] layoutWithOffset, FieldInfo fieldInfo, PropertyInfo propertyInfo)
        {
            Type = type;
            Name = name;
            FullName = fullName;
    ...
    public class EditorEntityDataMeta
    {
        ...
        public EditorEntityDataMeta(Type type, string name, string fullName, float width, GUILayoutOption[] layout,
            GUILayoutOption[] layoutWithOffset, FieldInfo fieldInfo, PropertyInfo propertyInfo)
        {
            Type = type;
            Name = name;
