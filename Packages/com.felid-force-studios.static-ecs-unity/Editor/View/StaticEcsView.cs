...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
        ...
        where TEventProvider : StaticEcsEventProvider<TWorld>
    {
        ...
        readonly List<IStaticEcsViewTab> _tabs = new();
        float _acc;

        AbstractWorldData _currentWorldData;

        bool _initialized;
        string _pendingTabSwitch;
        float _saveAcc;
        IStaticEcsViewTab _selectedTab;
        string _worldKey;
        ...
        void OnDisable()
        {
            ...
            EditorApplication.playModeStateChanged -= OnPlayModeStateChanged;
            SaveAllTabs();
            foreach (var tab in _tabs) tab.Destroy();
        ...
        void OnGUI()
        {
            ...
            {
                if (_currentWorldData != worldData) SetWorldData(worldData);
            }
            else
            {
            ...
            if (_currentWorldData == null)
            {
                EditorGUILayout.HelpBox(
                    $"World {typeof(TWorld).Name} is not registered. Call EcsDebug<{typeof(TWorld).Name}>.AddWorld()",
                    MessageType.Warning);
                return;
            }
        ...
        public void ReloadFromConfig(StaticEcsViewConfig config)
        {
            ...
            foreach (var tab in _tabs) tab.LoadState(worldSettings);
        ...
        public void SaveAllTabs()
        {
            ...
            foreach (var tab in _tabs) tab.SaveState(worldSettings);
            config.Save();
        }
        ...
        public void Init()
        {
            if (!_initialized || _tabs.Count == 0)
            {
                ...
                if (_pendingTabSwitch == null && _tabs.Count > 0) _selectedTab = _tabs[0];

                EntityInspectorRegistry.ShowEntityHandlers[typeof(TWorld)] =
                    EntityInspectorHelper<TWorld, TEntityProvider>.ShowWindowForEntity;
        ...
        void SetWorldData(AbstractWorldData data)
        {
            MetaData.EnrichByWorld(data.Handle);

            _currentWorldData = data;

            foreach (var tab in _tabs) tab.OnWorldChanged(_currentWorldData);
        }
