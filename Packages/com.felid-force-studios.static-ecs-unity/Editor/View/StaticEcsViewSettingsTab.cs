        ...
        where TWorld : struct, IWorldType
    {
        ...
        void DrawConfigSection()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                var newConfig = (StaticEcsViewConfig)EditorGUILayout.ObjectField(current, typeof(StaticEcsViewConfig),
                    false, Ui.WidthLine(250));
                if (newConfig != null && newConfig != current)
                {
                    if (_parentWindow is IStaticEcsViewConfigHost host) host.SaveAllTabs();
                    StaticEcsViewConfig.SetActive(newConfig);
                    if (_parentWindow is IStaticEcsViewConfigHost host2) host2.ReloadFromConfig(newConfig);
                }
            }
            EditorGUILayout.EndHorizontal();
        }

        void DrawFoldoutSection()
        {
        ...
        void DrawResetSection()
        {
            ...
            if (GUILayout.Button("Reset config to defaults", Ui.ButtonStyleTheme, Ui.WidthLine(200)))
            {
                ...
                if (_parentWindow is IStaticEcsViewConfigHost host) host.ReloadFromConfig(config);
            }
        }
    }
