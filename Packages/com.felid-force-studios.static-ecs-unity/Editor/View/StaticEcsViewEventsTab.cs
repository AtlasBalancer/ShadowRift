...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        static readonly TabType[] _tabs = { TabType.Table, TabType.EventBuilder };
        static readonly string[] _tabsNames = { "Table", "Event builder" };

        readonly Dictionary<Type, EventsDrawer<TWorld, TEventsProvider>> _drawersByWorldTypeType = new();
        EventsDrawer<TWorld, TEventsProvider> _currentDrawer;

        EventsSettings _savedSettings;
        ...
        public void OnWorldChanged(AbstractWorldData newWorldData)
        {
            if (!_drawersByWorldTypeType.ContainsKey(newWorldData.Handle.WorldType))
                _drawersByWorldTypeType[newWorldData.Handle.WorldType] =
                    new EventsDrawer<TWorld, TEventsProvider>(this, newWorldData, _savedSettings);
    ...
    {
        const float _maxWidth = 1056f;

        readonly TEventProvider _builder;
        readonly Dictionary<Type, EditorEventDataMetaByWorld> _eventsByType = new();
        readonly EditorEventDataMetaByWorld[] _eventsMeta;

        // filter
        readonly List<EditorEventDataMetaByWorld> _filterTypes = new();

        readonly StaticEcsViewEventsTab<TWorld, TEventProvider> _parent;
        readonly AbstractWorldData _worldData;
        bool _showAfterBuild;
        PageView<EventData> currentFilteredPage;
        PageView<EventData> currentPage;

        readonly PageRingBuffer<EventData> events;
        readonly PageRingBuffer<EventData> filteredEvents;
            ...
            EventsSettings savedSettings)
        {
            ...
            {
                if (_filterTypes.Count > 0 && _filterTypes.Contains(_eventsByType[item.TypeIdx.Type]))
                    filteredEvents.Change(item, (EventData template, ref EventData data) => { data = template; });
            });

            if (savedSettings != null) LoadFromConfig(savedSettings);
        }
        ...
        internal void SaveToConfig(EventsSettings settings)
        {
            ...
            foreach (var meta in _filterTypes) settings.filterTypeNames.Add(meta.FullName);
        }
        ...
        internal void LoadFromConfig(EventsSettings settings)
        {
            ...
            if (_filterTypes.Count > 0) UpdateFilteredPage();
        }
            ...
            switch (_parent.SelectedTab)
            {
                ...
                case StaticEcsViewEventsTab<TWorld, TEventProvider>.TabType.EventBuilder:
                    ...
                    Drawer.DrawEvent<TWorld, TEventProvider>(_builder, DrawMode.Builder, provider =>
                    {
                        provider.SendEvent();
                        if (_showAfterBuild)
                            EventInspectorHelper<TWorld, TEventProvider>.ShowWindowForEvent(_builder.RuntimeEvent,
                                _builder.EventCache);
        ...
        void DrawFilter<T>(ref PageView<T> pageView) where T : IEquatable<T>
        {
            Space(10);

            BeginHorizontal();
            {
                if (!pageView.IsActual) pageView.MoveToNewer();

                Latest = GUILayout.Toggle(Latest, "| <<<", Ui.ButtonStyleTheme, Ui.Width(60));
                while (Latest && pageView.HasNewer) pageView.MoveToNewer();

                using (Ui.EnabledScopeVal(pageView.HasNewer))
                {
                    if (GUILayout.Button("<-", Ui.ButtonStyleTheme, Ui.Width(60))) pageView.MoveToNewer();
                }
                ...
                using (Ui.EnabledScopeVal(pageView.HasOlder))
                {
                    ...
                    {
                        Latest = false;
                        while (pageView.HasOlder) pageView.MoveToOlder();
                    }
                }
                ...
                if (Ui.PlusButton)
                {
                    ...
                    foreach (var meta in _eventsMeta)
                    {
                        if (_filterTypes.Contains(meta)) continue;
                ...
                if (deleted) UpdateFilteredPage();
            }
            EndHorizontal();
            Space(20);
        }

        void UpdateFilteredPage()
        {
        ...
        void DrawRow(ref EventData val)
        {
            ...
            BeginHorizontal();
            {
                ...
                SelectableLabel(val.TypeIdx.Type.EditorTypeName(),
                    val.TypeIdx.Type.EditorTypeColor(out var color) ? Ui.LabelStyleThemeCenterColor(color) : style,
                    Ui.WidthLine(200));
                Ui.DrawSeparator();
                ...
                Ui.DrawSeparator();
                SelectableLabel(
                    Ui.IntToStringD6(val.EventStatus is EventStatus.Read or EventStatus.Suppressed
                        ? 0
                        : eventsHandle.UnreadCount(val.InternalIdx)).simple, style, Ui.WidthLine(60));
                Ui.DrawSeparator();
            }
            EndHorizontal();
        ...
        void DrawDeleteEventButton(ref EventData data)
        {
            LabelField(GUIContent.none, Ui.Width(10));
        ...
        void DrawViewEventButton(ref EventData data)
        {
            LabelField(GUIContent.none, Ui.Width(10));
            if (Ui.ViewButtonExpand) EventInspectorHelper<TWorld, TEventProvider>.ShowWindowForEvent(in data);
        }
        ...
        TEventProvider CreateEventView()
        {
