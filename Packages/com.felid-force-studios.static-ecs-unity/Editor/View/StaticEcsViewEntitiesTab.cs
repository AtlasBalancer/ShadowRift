...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        static readonly TabType[] _tabs = { TabType.Table, TabType.EntityBuilder };
        static readonly string[] _tabsNames = { "Table", "Entity builder" };

        readonly Dictionary<Type, EntitiesDrawer<TWorld, TEntityProvider>> _drawersByWorldTypeType = new();
        EntitiesDrawer<TWorld, TEntityProvider> _currentDrawer;

        EntitiesSettings _savedSettings;
        ...
        public void Destroy()
        {
            foreach (var drawer in _drawersByWorldTypeType) drawer.Value.Destroy();
        }
        ...
        public void OnWorldChanged(AbstractWorldData newWorldData)
        {
            if (!_drawersByWorldTypeType.ContainsKey(newWorldData.Handle.WorldType))
                _drawersByWorldTypeType[newWorldData.Handle.WorldType] =
                    new EntitiesDrawer<TWorld, TEntityProvider>(this, newWorldData, _savedSettings);
    ...
    public partial class EntitiesDrawer<TWorld, TEntityProvider> where TWorld : struct, IWorldType
        where TEntityProvider : StaticEcsEntityProvider<TWorld>
    {
        readonly List<EditorEntityDataMetaByWorld> _components = new();
        readonly List<EditorEntityDataMetaByWorld> _componentsColumns = new();

        readonly TEntityProvider _entityBuilder;

        readonly StaticEcsViewEntitiesTab<TWorld, TEntityProvider> _parent;
        readonly List<EntityGID> _pinedEntities = new();

        readonly List<EditorEntityDataMetaByWorld> _tags = new();
        readonly List<EditorEntityDataMetaByWorld> _tagsColumns = new();

        bool _gidFilterActive;
        int _gidFilterValue;
        int _lastSnapshotCount;

        float _maxWidth;

        EditorEntityDataMetaByWorld _sortIdx;
        Vector2 horizontalScrollEntitiesPosition = Vector2.zero;
        Vector2 verticalScrollEntitiesPosition = Vector2.zero;


        internal EntitiesDrawer(StaticEcsViewEntitiesTab<TWorld, TEntityProvider> parent, AbstractWorldData worldData,
            EntitiesSettings savedSettings)
        {
            _parent = parent;
        ...
        void ShowAllColumns(bool? showTableData)
        {
            _componentsColumns.Clear();
            foreach (var val in _components)
            {
                if (showTableData.HasValue) val.ShowTableData = showTableData.Value;
                _componentsColumns.Add(val);
            }

            _tagsColumns.Clear();
            foreach (var val in _tags) _tagsColumns.Add(val);
        }

        void ShowNoneColumns()
        {
            _componentsColumns.Clear();
            _tagsColumns.Clear();
        }
        ...
        internal void SaveToConfig(EntitiesSettings settings)
        {
            ...
            foreach (var col in _componentsColumns)
                if (col.ShowTableData)
                    settings.showTableDataTypes.Add(col.FullName);
        ...
        static void SaveFilterList(List<string> target, List<EditorEntityDataMetaByWorld> source)
        {
            target.Clear();
        ...
        internal void LoadFromConfig(EntitiesSettings settings)
        {
            ...
            foreach (var raw in settings.pinnedEntities) _pinedEntities.Add(new EntityGID(raw));
        ...
        void LoadFilterList(List<string> source, List<EditorEntityDataMetaByWorld> target)
        {
            target.Clear();
        ...
        static EditorEntityDataMetaByWorld FindByFullName(List<EditorEntityDataMetaByWorld> list, string fullName)
        {
        ...
        void DrawEntitiesTable()
        {
            horizontalScrollEntitiesPosition = GUILayout.BeginScrollView(horizontalScrollEntitiesPosition);
            DrawHeaders();
            DrawPined();

            verticalScrollEntitiesPosition =
                GUILayout.BeginScrollView(verticalScrollEntitiesPosition, Ui.Width(_maxWidth + 25f));
            _currentEntityCount = 0;

            if (_gidFilterActive)
            {
                var gidEntity = new World<TWorld>.Entity((uint)_gidFilterValue);
                if (!gidEntity.IsDestroyed) DrawEntityRow(gidEntity, false, false);
            }
            else
            {
                ...
                if (snapshot != null)
                {
                    if (Event.current.type == EventType.Layout) _lastSnapshotCount = snapshot.Count;
        ...
        void DrawPined()
        {
            ...
            if (_pinedEntities.Count > 0) Ui.DrawHorizontalSeparator(_maxWidth);
        }
        ...
        void DrawHeaders()
        {
            ...
            BeginHorizontal();
            {
                ...
                for (var i = 0; i < _tagsColumns.Count;)
                {
                    ...
                    SelectableLabel(idx.Name,
                        idx.Type.EditorTypeColor(out var color)
                            ? Ui.LabelStyleThemeCenterColor(color)
                            : Ui.LabelStyleThemeCenter, idx.Layout);
                    DrawSortButton(idx);
        ...
        void DrawComponents(List<EditorEntityDataMetaByWorld> columns)
        {
            const string DataOn = "☑";
            ...
            for (var i = 0; i < columns.Count;)
            {
                ...
                {
                    if (GUILayout.Button(DataOn, Ui.ButtonIconStyleGreen, Ui.WidthLine(21))) idx.ShowTableData = false;
                }
                else
                {
                    if (GUILayout.Button(DataOff, Ui.ButtonIconStyleTheme, Ui.WidthLine(21))) idx.ShowTableData = true;
                }
        ...
        void DrawSortButton(EditorEntityDataMetaByWorld idx)
        {
            const string Label = "⇧";

            if (idx == _sortIdx)
            {
                if (GUILayout.Button(Label, Ui.ButtonIconStyleGreen, Ui.WidthLine(20))) _sortIdx = null;
            }
            else
            {
                if (GUILayout.Button(Label, Ui.ButtonIconStyleTheme, Ui.WidthLine(20))) _sortIdx = idx;
            }
        }

        void DrawDeleteColumnButton<T>(ref int i, List<T> values)
        {
            const string Label = "✖";
        ...
        bool DrawEntityRow(World<TWorld>.Entity entity, bool sorted, bool pined)
        {
            if (_currentEntityCount >= _maxEntityResult && !pined) return false;

            if (entity.IsDestroyed || (!pined && _pinedEntities.Contains(entity))) return true;
        ...
        void DrawComponents(World<TWorld>.Entity entity)
        {
            _maxWidth = 180f;
        ...
        static void DrawEntityId(World<TWorld>.Entity entity)
        {
            SelectableLabel(Ui.IntToStringD6((int)entity.ID).d6, Ui.LabelStyleThemeCenter, Ui.WidthLine(60));
            Ui.DrawSeparator();
        }

        bool DrawDeleteEntityButton(World<TWorld>.Entity entity)
        {
        ...
        void DrawPinEntityButton(World<TWorld>.Entity entity, bool pinned)
        {
        ...
        void DrawViewEntityButton(World<TWorld>.Entity entity)
        {
            LabelField(GUIContent.none, Ui.Width(10));
            if (Ui.ViewButtonExpand) EntityInspectorHelper<TWorld, TEntityProvider>.ShowWindowForEntity(entity.GID);
        }

        void DrawComponents(World<TWorld>.Entity entity, List<EditorEntityDataMetaByWorld> types, float widthAdd)
        {
            ...
            foreach (var idx in types)
            {
                if (idx.HasComponent(entity))
                {
                    ...
                    if (idx.ShowTableData)
                    {
                        if (idx.ComponentHandle.HasValue &&
                            idx.ComponentHandle.Value.TryGetRaw(entity.ID, out var rawValue))
                        {
    ...
    {
        readonly Func<uint, bool> _hasComponentFunc;
        public readonly ComponentsHandle? ComponentHandle;
        public readonly ComponentsHandle? TagHandle;
        public bool ShowTableData;

        public EditorEntityDataMetaByWorld(EditorEntityDataMeta meta, ComponentsHandle? componentHandle,
            ComponentsHandle? tagHandle, Func<uint, bool> hasComponentFunc)
            : base(meta.Type, meta.Name, meta.FullName, meta.Width, meta.Layout, meta.LayoutWithOffset, meta.FieldInfo,
                meta.PropertyInfo)
        {
        ...
        public bool HasComponent<TWorld>(World<TWorld>.Entity entity) where TWorld : struct, IWorldType
        {
            return _hasComponentFunc(entity.ID);
        }
