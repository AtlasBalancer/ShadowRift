...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        readonly List<EditorEntityDataMetaByWorld> _all = new();
        readonly List<EditorEntityDataMetaByWorld> _allOnlyDisabled = new();
        readonly List<EditorEntityDataMetaByWorld> _allWithDisabled = new();
        readonly List<EditorEntityDataMetaByWorld> _any = new();
        readonly List<EditorEntityDataMetaByWorld> _anyOnlyDisabled = new();
        readonly List<EditorEntityDataMetaByWorld> _anyWithDisabled = new();
        readonly List<EditorEntityDataMetaByWorld> _componentsAndTags = new();
        readonly List<EditorEntityDataMetaByWorld> _none = new();
        readonly List<EditorEntityDataMetaByWorld> _noneWithDisabled = new();
        CompositeHandleFilter _cachedFilter;
        int _currentEntityCount;
        bool _filterActive;
        bool _filterDirty = true;
        int _maxEntityResult = 100;

        void DrawEntitiesFilter()
        {
            SegmentFilter();
            ComponentsFilter();
            GidFilter();
        ...
        void SegmentFilter()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Clear", Ui.ButtonStyleTheme, Ui.WidthLine(60))) system.ClearSegmentFilter();
            }
            EditorGUILayout.EndHorizontal();
        }

        void ComponentsFilter()
        {
            EditorGUILayout.BeginHorizontal();
            {
            ...
            if (_filterActive)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _all);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_components, _allOnlyDisabled);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _allWithDisabled);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _none);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _noneWithDisabled);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _any);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_components, _anyOnlyDisabled);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (Ui.PlusButton) DrawShowFilterMenu(_componentsAndTags, _anyWithDisabled);
        ...
        bool IsFilterValid()
        {
            return _filterActive && (_all.Count > 0 || _allOnlyDisabled.Count > 0 || _allWithDisabled.Count > 0 ||
                                     _none.Count > 0 || _noneWithDisabled.Count > 0 || _any.Count > 1
                    ...
                    if (Ui.PlusDropDownButton)
                    {
                        ...
                        foreach (var idx in _components)
                            if (!_componentsColumns.Contains(idx))
                                menu.AddItem(new GUIContent(idx.FullName), false,
                                    objType => _componentsColumns.Add((EditorEntityDataMetaByWorld)objType), idx);

                        foreach (var idx in _tags)
                            if (!_tagsColumns.Contains(idx))
                                menu.AddItem(new GUIContent(idx.FullName), false,
                                    objType => _tagsColumns.Add((EditorEntityDataMetaByWorld)objType), idx);
        ...
        CompositeHandleFilter GetOrBuildFilter()
        {
            ...
            if (_all.Count > 0)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_all), QueryMethodType.ALL));
            if (_allOnlyDisabled.Count > 0)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_allOnlyDisabled),
                    QueryMethodType.ALL_ONLY_DISABLED));
            if (_allWithDisabled.Count > 0)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_allWithDisabled),
                    QueryMethodType.ALL_WITH_DISABLED));
            if (_none.Count > 0)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_none), QueryMethodType.NONE));
            if (_noneWithDisabled.Count > 0)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_noneWithDisabled),
                    QueryMethodType.NONE_WITH_DISABLED));
            if (_any.Count > 1)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_any), QueryMethodType.ANY));
            if (_anyOnlyDisabled.Count > 1)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_anyOnlyDisabled),
                    QueryMethodType.ANY_ONLY_DISABLED));
            if (_anyWithDisabled.Count > 1)
                _cachedFilter.Add(new HandleComponentsFilter(ExtractComponentHandles(_anyWithDisabled),
                    QueryMethodType.ANY_WITH_DISABLED));
            return _cachedFilter;
        }

        static List<ComponentsHandle> ExtractComponentHandles(List<EditorEntityDataMetaByWorld> list)
        {
        ...
        void DrawFilterLabels(List<EditorEntityDataMetaByWorld> types)
        {
        ...
        void DrawShowFilterMenu(List<EditorEntityDataMetaByWorld> types, List<EditorEntityDataMetaByWorld> result)
        {
            var menu = new GenericMenu();
