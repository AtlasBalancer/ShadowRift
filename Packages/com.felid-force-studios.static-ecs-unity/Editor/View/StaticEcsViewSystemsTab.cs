...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        static readonly TabType[] _tabs = { TabType.Init, TabType.Update, TabType.Destroy };
        static readonly string[] _tabsNames = { "Init", "Update", "Destroy" };
        SystemDrawer<TWorld> _currentDrawer;
        AbstractWorldData _currentWorldData;

        readonly Dictionary<Type, SystemDrawer<TWorld>> _drawersBySystemIdType = new();

        SystemsSettings _savedSettings;
        ...
        {
            CheckDrawer();
            if (_currentDrawer != null) DrawSystemsSelector();
        }
            ...
            if (_currentDrawer == null && _currentWorldData != null)
            {
                ...
                foreach (var typeToSystem in StaticEcsDebugData.Systems)
                    if (typeToSystem.Value.worldType == _currentWorldData.Handle.WorldType &&
                        !_drawersBySystemIdType.ContainsKey(typeToSystem.Key))
                    {
        ...
        {
            if (GUILayout.Button(new GUIContent("Systems:", EditorGUIUtility.IconContent("d_Preset.Context").image),
                    Ui.IconButtonStretchedStyle, Ui.ExpandWidthFalse()))
            {
                var menu = new GenericMenu();
                foreach (var drawer in _drawersBySystemIdType)
                    if (drawer.Value.Systems.worldType == _currentWorldData.Handle.WorldType &&
                        drawer.Value != _currentDrawer)
                        menu.AddItem(new GUIContent(drawer.Value.SysIdType.EditorTypeName()), false,
                            () => { _currentDrawer = drawer.Value; });
    ...
    {
        static readonly Dictionary<int, string> _formattedTime = new();
        int _drawLevel = 10;
        ...
        static string FormatTime(float avgTime)
        {
        ...
        bool DrawSystemNameFoldout(int index, SystemData systemData)
        {
        ...
        void DrawSystemFields(SystemData systemData)
        {
            var systemType = systemData.System.GetType();
            var systemName = systemType.EditorTypeName();
            _drawLevel = 10;
            ...
            switch (Parent.SelectedTab)
            {
                case StaticEcsViewSystemsTab<TWorld>.TabType.Init:
                    ...
                    for (var i = 0; i < Systems.count; i++)
                    {
                        ...
                        if (systemData.HasInit)
                        {
                            ...
                            Ui.DrawSeparator();
                            EditorGUILayout.EndHorizontal();
                            if (isOpen) DrawSystemFields(systemData);
                            Ui.DrawHorizontalSeparator(410f);
                        }
                    }
                ...
                case StaticEcsViewSystemsTab<TWorld>.TabType.Update:
                    ...
                    for (var i = 0; i < Systems.count; i++)
                    {
                        ...
                        if (newActive != effectiveActive) systemData.DebugDisabled = !newActive;
                        EditorGUILayout.LabelField(GUIContent.none, Ui.LabelStyleThemeBold, Ui.WidthLine(10));
                        Ui.DrawSeparator();
                        ...
                        EditorGUILayout.LabelField(FormatTime(systemData.AvgUpdateTime), Ui.LabelStyleThemeBold,
                            Ui.WidthLine(60));
                        Ui.DrawSeparator();
                        EditorGUILayout.EndHorizontal();
                        if (isOpen) DrawSystemFields(systemData);
