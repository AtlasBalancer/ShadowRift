...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public class StartupTemplateWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (string.IsNullOrEmpty(startupName)) startupName = "Startup";
            worldTypeName = EditorGUILayout.TextField("World Type Name", worldTypeName);
            if (string.IsNullOrEmpty(worldTypeName)) worldTypeName = "WT";
            worldAliasName = EditorGUILayout.TextField("World Alias Name", worldAliasName);
            if (string.IsNullOrEmpty(worldAliasName)) worldAliasName = "W";
            ...
            if (updateSystems)
            {
                ...
                if (string.IsNullOrEmpty(updateSystemsTypeName)) updateSystemsTypeName = "UpdateSystemsType";
                updateSystemsAliasName = EditorGUILayout.TextField("Systems Alias Name", updateSystemsAliasName);
                if (string.IsNullOrEmpty(updateSystemsAliasName)) updateSystemsAliasName = "UpdateSystems";
                EditorGUI.indentLevel--;
            }
            ...
            if (fixedUpdateSystems)
            {
                ...
                fixedUpdateSystemsAliasName =
                    EditorGUILayout.TextField("Systems Alias Name", fixedUpdateSystemsAliasName);
            ...
            if (lateUpdateSystems)
            {
                ...
                lateUpdateSystemsAliasName =
                    EditorGUILayout.TextField("Systems Alias Name", lateUpdateSystemsAliasName);
                if (string.IsNullOrEmpty(lateUpdateSystemsAliasName)) lateUpdateSystemsAliasName = "LateUpdateSystems";
                EditorGUI.indentLevel--;
            }
        }
        ...
        public string CreateTemplate()
        {
            ...
            sb.AppendLine(
                $"public abstract class {updateSystemsAliasName} : {worldAliasName}.Systems<{updateSystemsTypeName}> {{ }}",
                updateSystems);
            sb.AppendLine(
                $"public abstract class {fixedUpdateSystemsAliasName} : {worldAliasName}.Systems<{fixedUpdateSystemsTypeName}> {{ }} ",
                fixedUpdateSystems);
            sb.AppendLine(
                $"public abstract class {lateUpdateSystemsAliasName} : {worldAliasName}.Systems<{lateUpdateSystemsTypeName}> {{ }}",
                lateUpdateSystems);
            sb.AppendLine();
            sb.AppendLine();
            ...
            sb.AppendLine(
                $"{pad}        // ============================================ MAIN INITIALIZATION ======================================================");
            sb.AppendLine($"{pad}        {worldAliasName}.Create(WorldConfig.Default());");
            if (debugSystemsType != null) sb.AppendLine($"{pad}        {debugSystemsTypeAlias}.Create();");
            sb.AppendLine();
            sb.AppendLine($"{pad}        // Types are auto-discovered via Types().RegisterAll()");
            sb.AppendLine(
                $"{pad}        // For manual registration use: {worldAliasName}.Types().Component<T>().Tag<T>().Event<T>();");
            sb.AppendLine();
            ...
            {
                sb.AppendLine(
                    $"{pad}        // ============================================ FIXED SYSTEMS INITIALIZATION ==============================================");
                if (fixedUpdateSystemsAliasName != debugSystemsTypeAlias)
                    sb.AppendLine($"{pad}        {fixedUpdateSystemsAliasName}.Create();");
                sb.AppendLine(
                    $"{pad}        // {fixedUpdateSystemsAliasName}.Add(new YourSystem1()).Add(new YourSystem2()).Add(new YourSystem3());");
                sb.AppendLine();
            ...
            {
                sb.AppendLine(
                    $"{pad}        // ============================================ LATE SYSTEMS INITIALIZATION ==============================================");
                if (lateUpdateSystemsAliasName != debugSystemsTypeAlias)
                    sb.AppendLine($"{pad}        {lateUpdateSystemsAliasName}.Create();");
                sb.AppendLine(
                    $"{pad}        // {lateUpdateSystemsAliasName}.Add(new YourSystem1()).Add(new YourSystem2()).Add(new YourSystem3());");
                sb.AppendLine();
