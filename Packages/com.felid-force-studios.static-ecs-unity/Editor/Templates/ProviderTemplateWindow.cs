...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public class ProviderTemplateWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                if (Ui.SettingButton) DrawWorldMenu();
                EditorGUILayout.LabelField("World:", Ui.WidthLine(60));
                EditorGUILayout.LabelField(worldName, Ui.LabelStyleThemeBold);
                if (worldType != null && !string.IsNullOrEmpty(prefix) &&
                    GUILayout.Button("Create Providers", Ui.ButtonStyleYellow))
                {
                    CreateFiles();
                    Close();
                }
            ...
            if (string.IsNullOrEmpty(prefix)) prefix = ShortName(worldName) ?? "Ecs";

            EditorGUILayout.Space(10);
            menuItemPath = EditorGUILayout.TextField("Menu Item Path", menuItemPath);
            if (string.IsNullOrEmpty(menuItemPath)) menuItemPath = "Window/" + prefix + " ECS";
            ...
            if (eventCount > 0) EditorGUILayout.LabelField("Events:", $"Events/ ({eventCount} providers)");
            EditorGUI.indentLevel--;
        ...
        public void CreateFiles()
        {
            ...
            WriteFile(editorPath, entityProviderEditorName,
                CreateEntityProviderEditorTemplate(entityProviderName, entityProviderEditorName));
            WriteFile(editorPath, eventProviderEditorName,
                CreateEventProviderEditorTemplate(eventProviderName, eventProviderEditorName));
            WriteFile(editorPath, ecsViewName,
                CreateEcsViewTemplate(entityProviderName, eventProviderName, ecsViewName));
        ...
        {
            if (!generateGUI && !generateTMP && !generateMouse && !generatePhysics3D && !generatePhysics2D &&
                !generateAnimation) return;
        ...
        void GenerateEventProviderTriple(string dir, string entityProviderName, string name)
        {
            ...
            WriteFile(dir, className + "EntityRef",
                CreateSealedProviderFile(className + "EntityRef",
                    $"{name}EntityRefProvider<{worldTypeName}, {entityProviderName}>"));
        }
        ...
        void GenerateStateMachineBehaviourGroup(string dir, string entityProviderName)
        {
            ...
            WriteFile(dir, className + "Entity",
                CreateSealedProviderFile(className + "Entity",
                    $"StaticEcsEntityStateMachineBehaviour<{worldTypeName}>"));

            WriteFile(dir, className + "Linker",
                CreateSealedProviderFile(className + "Linker",
                    $"StaticEcsStateMachineBehaviourLinker<{worldTypeName}, {entityProviderName}>"));
        }
        ...
        void GenerateContactColliderProviderPair(string dir, string entityProviderName)
        {
            ...
            WriteFile(dir, className + "EntityRef",
                CreateSealedProviderFile(className + "EntityRef",
                    $"ContactColliderRefProvider<{worldTypeName}, {entityProviderName}>"));
        }
        ...
        public string CreateEntityProviderEditorTemplate(string providerName, string editorName)
        {
            ...
            if (!string.IsNullOrEmpty(nameSpace)) sb.AppendLine($"using {nameSpace};");
            sb.AppendLine();
            ...
            sb.AppendLine(
                $"{pad}public class {editorName} : StaticEcsEntityProviderEditor<{worldTypeName}, {providerName}> {{ }}");
            sb.AppendLine("}", !string.IsNullOrEmpty(nameSpace));
            return sb.ToString();
        }
        ...
        public string CreateEventProviderEditorTemplate(string providerName, string editorName)
        {
            ...
            sb.AppendLine($"{pad}[CustomEditor(typeof({providerName})), CanEditMultipleObjects]");
            sb.AppendLine(
                $"{pad}public class {editorName} : StaticEcsEvenTEntityProviderEditor<{worldTypeName}, {providerName}> {{ }}");
            sb.AppendLine("}", !string.IsNullOrEmpty(nameSpace));
            return sb.ToString();
        }
        ...
        public string CreateEcsViewTemplate(string entityProviderName, string eventProviderName, string viewName)
        {
            ...
            sb.AppendLine($"namespace {nameSpace}.Editor {{", !string.IsNullOrEmpty(nameSpace));
            sb.AppendLine(
                $"{pad}public class {viewName} : StaticEcsView<{worldTypeName}, {entityProviderName}, {eventProviderName}> {{");
