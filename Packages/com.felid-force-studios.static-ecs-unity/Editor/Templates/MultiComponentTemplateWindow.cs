...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public class MultiComponentTemplateWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (worldType != null && names.Length > 0 &&
                    GUILayout.Button("Create Multi-Components", Ui.ButtonStyleYellow))
                {
                    CreateFiles();
                    Close();
                }
            ...
            for (var i = 0; i < names.Length; i++)
            {
                ...
                if (string.IsNullOrEmpty(val)) val = "Element";
            }
        ...
        public string CreateTemplate(string elementName)
        {
            ...
            {
                sb.AppendLine(
                    $"{pad}    public static readonly ComponentTypeConfig<Multi<{elementName}>> Config = new(");
                sb.AppendLine($"{pad}        guid: new(\"{GUID.Generate().ToString()}\"),");
                sb.AppendLine(
                    $"{pad}        readWriteStrategy: new MultiUnmanagedPackArrayStrategy<{worldTypeName}, {elementName}>()");
                sb.AppendLine($"{pad}    );");
                sb.AppendLine();
            }
            ...
            {
                sb.AppendLine(
                    $"{pad}    public static readonly IPackArrayStrategy<{elementName}> PackStrategy = new UnmanagedPackArrayStrategy<{elementName}>();");
                sb.AppendLine();
            }
            ...
            {
                sb.AppendLine(
                    $"{pad}    public static readonly IPackArrayStrategy<{elementName}> PackStrategy = new StructPackArrayStrategy<{elementName}>();");
                sb.AppendLine();
            }
