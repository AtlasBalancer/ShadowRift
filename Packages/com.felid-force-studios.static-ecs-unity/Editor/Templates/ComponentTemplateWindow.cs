...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    internal static class Extensions
    {
        ...
        {
            if (val) builder.AppendLine(line);
        }
    }
    ...
    public class ComponentTemplateWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (worldType != null && names.Length > 0 &&
                    GUILayout.Button("Create Components", Ui.ButtonStyleYellow))
                {
                    CreateFiles();
                    Close();
                }
            ...
            for (var i = 0; i < names.Length; i++)
            {
                ...
                if (string.IsNullOrEmpty(val)) val = "Component";
            }
        ...
        public string CreateTemplate(string componentName)
        {
            ...
            if (serialization)
            {
                ...
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void Write<TWorld>(ref BinaryPackWriter writer, World<TWorld>.Entity self) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        throw new NotImplementedException(); // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void Read<TWorld>(ref BinaryPackReader reader, World<TWorld>.Entity self, byte version, bool disabled) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        throw new NotImplementedException(); // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
            }
            ...
            {
                sb.AppendLine(
                    $"{pad}    public void OnAdd<TWorld>(World<TWorld>.Entity self) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void OnDelete<TWorld>(World<TWorld>.Entity self, HookReason reason) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void CopyTo<TWorld>(World<TWorld>.Entity self, World<TWorld>.Entity other, bool disabled) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
            }
