...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public class EntityLinksComponentTemplateWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (worldType != null && names.Length > 0 &&
                    GUILayout.Button("Create Links-Components", Ui.ButtonStyleYellow))
                {
                    CreateFiles();
                    Close();
                }
            ...
            for (var i = 0; i < names.Length; i++)
            {
                ...
                if (string.IsNullOrEmpty(val)) val = "Links";
            }
        ...
        public string CreateTemplate(string linksName)
        {
            ...
            sb.AppendLine($"{pad}public struct {linksName} : ILinksType {{");
            if (serialization)
            {
                sb.AppendLine($"{pad}    public static readonly ComponentTypeConfig<Links<{linksName}>> Config = new(");
                sb.AppendLine($"{pad}        guid: new(\"{GUID.Generate().ToString()}\"),");
                sb.AppendLine(
                    $"{pad}        readWriteStrategy: new LinksUnmanagedPackArrayStrategy<{worldTypeName}, {linksName}>()");
                sb.AppendLine($"{pad}    );");
                sb.AppendLine();
            }
            ...
            {
                sb.AppendLine(
                    $"{pad}    public void OnAdd<TWorld>(World<TWorld>.Entity self, EntityGID link) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void OnDelete<TWorld>(World<TWorld>.Entity self, EntityGID link, HookReason reason) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
                sb.AppendLine();
                sb.AppendLine(
                    $"{pad}    public void CopyTo<TWorld>(World<TWorld>.Entity self, World<TWorld>.Entity other, EntityGID link) where TWorld : struct, IWorldType {{");
                sb.AppendLine($"{pad}        // TODO implement this");
                sb.AppendLine($"{pad}    }}");
            }
