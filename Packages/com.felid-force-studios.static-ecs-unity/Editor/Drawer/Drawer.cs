...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public static partial class Drawer
    {
        ...
        public static void DrawFoldoutBox(int keyHash, string open, string close, out bool show, GUIStyle style)
        {
            ...
            using (Ui.EnabledScope)
            {
                if (!openHideFlags.Contains(keyHash))
                {
                    ...
                    if (GUI.Button(rect, $"▸ {open}", style)) openHideFlags.Add(keyHash);
                ...
                {
                    EditorGUILayout.BeginVertical(GUI.skin.box);
                    if (GUI.Button(rect, $"▾ {close}", style)) openHideFlags.Remove(keyHash);
        ...
        internal static void DrawProperty(object component, PropertyInfo field, GUIStyle style,
        ...
        static void DrawTableValue(GUIStyle style, GUILayoutOption[] layout, Type fieldType, object fieldValue)
        {
            ...
            if (strVal.Length > MaxFieldToStringLength) strVal = strVal.Substring(0, MaxFieldToStringLength);
        ...
        public static bool TryDrawObject(ref int level, string name, Type type, object value, out object newValue)
        {
            ...
            if (type != typeof(string) && level >= 0)
            {
                ...
                if (fields.Length > 0)
                {
                    ...
                    foreach (var field in fields)
                    {
                        ...
                        if (TryDrawObject(ref level, field.Name, fValue?.GetType() ?? field.FieldType, fValue,
                                out newValue))
                        {
            ...
            if (level < 0) DrawSelectableText(name, "Display level of nested objects exceeded!");
