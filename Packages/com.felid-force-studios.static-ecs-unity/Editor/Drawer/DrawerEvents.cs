...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    public static partial class Drawer
    {
        ...
        {
            if (mode != DrawMode.Inspector) provider.Scroll = EditorGUILayout.BeginScrollView(provider.Scroll);
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (Ui.PlusButton && allowChangeEventType) DrawEventsMenu<TWorld, TProvider>(provider);
                }
                ...
                {
                    EditorGUILayout.LabelField(provider.EventTemplate.GetType().EditorTypeName(),
                        Ui.LabelStyleThemeBold, Ui.WidthLine(200));
            ...
            if (!provider.RuntimeEvent.IsEmpty())
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    EditorGUILayout.LabelField(provider.IsCached() ? "Read or suppressed" : "Sent",
                        Ui.LabelStyleThemeBold);
                }
                EditorGUILayout.EndHorizontal();
            }
        ...
        static void DrawEventValue<TWorld, TEntityProvider>(TEntityProvider provider)
            where TEntityProvider : StaticEcsEventProvider<TWorld> where TWorld : struct, IWorldType
        {
        ...
        static void DrawEventsMenu<TWorld, TEntityProvider>(TEntityProvider provider)
            where TEntityProvider : StaticEcsEventProvider<TWorld> where TWorld : struct, IWorldType
        {
            ...
            {
                if (provider.EventTemplate != null && provider.EventTemplate.GetType() == eventDataMeta.Type) continue;
