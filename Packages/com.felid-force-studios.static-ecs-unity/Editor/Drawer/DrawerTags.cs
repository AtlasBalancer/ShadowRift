...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        static void DrawTags<TWorld>(List<IComponentOrTagProvider> providers, Object obj,
            StaticEcsEntityProvider<TWorld> provider) where TWorld : struct, IWorldType
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (Ui.PlusDropDownButton && !hasAll) DrawTagsMenu(providers, obj, provider);
                }
            ...
            for (var o = 0; o < providers.Count; o++)
            {
                ...
                EditorGUILayout.BeginHorizontal(GUI.skin.box);
                {
                    if (colored)
                        ...
                        EditorGUILayout.SelectableLabel(type.EditorTypeName(), EditorStyles.boldLabel,
                            GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight));
                    if (Ui.TrashButton)
                    {
        ...
        static void DrawTagsMenu<TWorld>(List<IComponentOrTagProvider> actualProviders, Object obj,
            StaticEcsEntityProvider<TWorld> provider) where TWorld : struct, IWorldType
        {
            var worldMeta = MetaData.GetWorldMetaData(typeof(TWorld));
