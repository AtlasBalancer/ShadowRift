...
namespace FFS.Libraries.StaticEcs.Unity.Editor
{
    ...
    {
        static readonly List<IComponentOrTagProvider> _providersCache = new();
        static readonly List<IComponentOrTagProvider> _componentProvidersCache = new();
        static readonly List<IComponentOrTagProvider> _tagProvidersCache = new();

        public static void DrawEntity<TWorld, TEntityProvider>(
            TEntityProvider provider, DrawMode mode, Action<TEntityProvider> onClickSpawn,
            Action<TEntityProvider> onClose
        ) where TEntityProvider : StaticEcsEntityProvider<TWorld> where TWorld : struct, IWorldType
        {
            ...
            {
                if (mode != DrawMode.Inspector) activeBase.Scroll = EditorGUILayout.BeginScrollView(activeBase.Scroll);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (active.EntityIsActual())
                    {
                        EditorGUILayout.SelectableLabel(Ui.IntToStringD6((int)active.EntityGid.Id).d6,
                            Ui.LabelStyleThemeBold, Ui.WidthLine(120));
                    ...
                    else
                    {
                        ...
                        using (Ui.EnabledScope)
                        {
                            ...
                            if (mode != DrawMode.Viewer && Application.isPlaying && active.HasComponents() &&
                                GUILayout.Button(text, Ui.ButtonStyleYellow, Ui.WidthLine(60))) onClickSpawn(origin);
                        }
                    }
                }
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (active.EntityIsActual())
                    {
                        ...
                        EditorGUILayout.SelectableLabel($"{typeName} ({entityTypeByte})", Ui.LabelStyleThemeBold,
                            Ui.WidthLine(120));
                    }
                    else
                    {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (active.EntityIsActual())
                        EditorGUILayout.SelectableLabel(Ui.IntToStringD6(active.EntityGid.ClusterId).d6,
                            Ui.LabelStyleThemeBold, Ui.WidthLine(120));
                    else
                ...
                if (mode != DrawMode.Inspector) EditorGUILayout.EndScrollView();
            }
        }

        static void DrawEntity<TWorld>(Object obj, StaticEcsEntityProvider<TWorld> provider)
