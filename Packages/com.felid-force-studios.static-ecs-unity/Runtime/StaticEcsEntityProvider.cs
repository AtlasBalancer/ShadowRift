...
namespace FFS.Libraries.StaticEcs.Unity
{
        ...
        where TWorld : struct, IWorldType
    {
        ...
        [SerializeReference] [HideInInspector] protected List<IComponentOrTagProvider> providers = new();
        ...
        protected void Awake()
        {
            ...
            {
                if (CreateEntity(false)) PostEntityProviderCreate.Instance.Register(this);
        ...
        public virtual bool CreateEntity(bool onCreateEntity = true)
        {
            ...
            if (disableEntityOnCreate) entityGid.Unpack<TWorld>().Disable();

            if (onCreateEntity) OnCreate();
            ...
            for (var i = 0; i < providers.Count; i++)
            {
                ...
                if (provider is LinkProvider { target: not null } lp)
                {
                    ...
                    if (world.TryGetComponentsHandle(lp.value.GetType(), out var handle))
                        handle.SetRaw(entity.ID, lp.value);
                }
                else if (provider is LinksProvider { targets: not null } lsp && lsp.targets.Count > 0 &&
                         lsp.value != null)
                {
                    if (world.TryGetComponentsHandle(lsp.value.GetType(), out var handle))
                    {
                        handle.SetRaw(entity.ID, lsp.value);
                        ...
                        if (hasRaw && raw is ILinksComponent linksComponent)
                        {
                            ...
                            {
                                var t = lsp.targets[j];
                                if (t != null) linksComponent.AddLink(t.EntityGid);
                            }
