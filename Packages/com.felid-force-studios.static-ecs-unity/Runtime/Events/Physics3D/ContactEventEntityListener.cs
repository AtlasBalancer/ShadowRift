...
namespace FFS.Libraries.StaticEcs.Unity
{
    ...
    {
        [SerializeField] bool sendNonEntityEvents;

        [SerializeField] bool manageComponents;
        ...
        void OnContactEvent(PhysicsScene scene, NativeArray<ContactPairHeader>.ReadOnly headers)
        {
            ...
            for (var h = 0; h < headers.Length; h++)
            {
                ...
                for (var p = 0; p < header.pairCount; p++)
                {
                    ...
                    if (pair.isCollisionExit) ProcessExit(ref map, colliderA, colliderB);
                }
            }
        }

        [MethodImpl(AggressiveInlining)]
        void ProcessEnter(ref ContactColliderEntityMap map, ContactPair pair, Collider colliderA, Collider colliderB)
        {
        ...
        [MethodImpl(AggressiveInlining)]
        void ProcessExit(ref ContactColliderEntityMap map, Collider colliderA, Collider colliderB)
        {
            if (sendNonEntityEvents)
