...
namespace FFS.Libraries.StaticEcs.Unity
{
    ...
    {
        public static bool IsComponent(this ComponentOrTagKind kind)
        {
            return (kind & ComponentOrTagKind.Component) != 0;
        }

        public static bool IsTag(this ComponentOrTagKind kind)
        {
            return (kind & ComponentOrTagKind.Tag) != 0;
        }

        public static bool IsMulti(this ComponentOrTagKind kind)
        {
            return (kind & ComponentOrTagKind.Multi) == ComponentOrTagKind.Multi;
        }

        public static bool IsLink(this ComponentOrTagKind kind)
        {
            return (kind & ComponentOrTagKind.Link) == ComponentOrTagKind.Link;
        }

        public static bool IsLinks(this ComponentOrTagKind kind)
        {
            return (kind & ComponentOrTagKind.Links) == ComponentOrTagKind.Links;
        }
    }
    ...
    public class ComponentProvider : IComponentOrTagProvider
    {
        ...
        public void Apply<TWorld>(World<TWorld>.Entity entity, bool deferred = false) where TWorld : struct, IWorldType
        {
            ...
            if (world.TryGetComponentsHandle(value.GetType(), out var handle)) handle.SetRaw(entity.ID, value);
        }
    }
    ...
    {
        [SerializeReference] public ITag value;
...
#endif
            ref var world = ref World<TWorld>.Data.Handle;
            if (world.TryGetComponentsHandle(value.GetType(), out var handle)) handle.Set(entity.ID);
        }
    }
    ...
    {
        [SerializeReference] public ILinkComponent value;

        [SerializeField] public AbstractStaticEcsEntityProvider target;
    ...
    {
        [SerializeReference] public ILinksComponent value;

        [SerializeField] public List<AbstractStaticEcsEntityProvider> targets = new();
