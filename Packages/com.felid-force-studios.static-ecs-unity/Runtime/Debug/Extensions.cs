        ...
        public static bool EditorTypeColor(this Type type, out Color color)
        {
            if (!_colorsCache.TryGetValue(type, out color))
            {
                ...
                foreach (var customAttribute in type.GetCustomAttributes())
                    if (customAttribute.GetType().Namespace + customAttribute.GetType().FullName ==
                        authAttrType.Namespace + authAttrType.FullName)
                    {
                        var fields = customAttribute.GetType()
                        ...
                        if (fields.Length == 3)
                        {
                            ...
                            if (floatCount == 3)
                            {
                                color = new Color((float)fields[0].GetValue(customAttribute),
                                    (float)fields[1].GetValue(customAttribute),
                                    (float)fields[2].GetValue(customAttribute), 1f);
        ...
        public static string EditorFullTypeName(this Type type)
        {
            if (!_fullNamesCache.TryGetValue(type, out var name))
            {
                ...
                if (Attribute.IsDefined(type, authAttrType))
                {
                    ...
                    if (string.IsNullOrEmpty(name)) name = type.FullName;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        for (var i = s.Length - 1; i >= 0; i--)
                        {
                            ...
                            if (started && (c == '[' || c == '.')) started = false;

                            if (i <= min || started) res.Append(c);
                        }
