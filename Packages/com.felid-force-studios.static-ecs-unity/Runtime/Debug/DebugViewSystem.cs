...
namespace FFS.Libraries.StaticEcs.Unity
{
    ...
    {
        readonly ConcurrentQueue<DebugCommand> _commandQueue = new();
        readonly long _refreshIntervalMs = 100;
        readonly Stopwatch _stopwatch = new();
        CompositeHandleFilter _filter;
        volatile int _maxEntityResult = 100;
        ComponentsHandle _segmentFilterHandle;
        EntitySnapshot<TWorld> _snapshot;
        ComponentsHandle? _sortComponentHandle;
        ...
        public bool IsSegmentFilterActive { get; private set; }

        public uint SegmentFilterIdx { get; private set; }
        ...
        {
            if (_stopwatch.ElapsedMilliseconds < _refreshIntervalMs) return false;
        ...
        {
            IsSegmentFilterActive = true;
            _segmentFilterHandle = handle;
            SegmentFilterIdx = segmentIdx;
        }

        public void ClearSegmentFilter()
        {
            IsSegmentFilterActive = false;
        }
        ...
        void ProcessCommands()
        {
            ...
            {
                if (!cmd.EntityGid.TryUnpack<TWorld>(out var entity)) continue;
        ...
        void RefreshSnapshot()
        {
            ...
            _snapshot.Count = 0;
            if (_snapshot.Entities.Length < max) Array.Resize(ref _snapshot.Entities, max);

            if (IsSegmentFilterActive)
            {
            ...
            if (_snapshot.Count < max)
            {
                ...
                if (filter.IsValid)
                    ...
                    {
                        if (_snapshot.Count >= max) break;
                        if (!IsAlreadyInSnapshot(entity, sortedCount)) _snapshot.Entities[_snapshot.Count++] = entity;
                    }
                    ...
                    {
                        if (_snapshot.Count >= max) break;
                        if (!HasSortHandle(entity)) _snapshot.Entities[_snapshot.Count++] = entity;
                    }
            }
        }

        void RefreshSnapshotBySegment(int max)
        {
            var segIdx = SegmentFilterIdx;
            ...
            for (var blockIdx = 0; blockIdx < Const.BLOCKS_IN_SEGMENT && _snapshot.Count < max; blockIdx++)
            {
                ...
                while (mask != 0 && _snapshot.Count < max)
                {
                    ...
                    if (!entity.IsDestroyed) _snapshot.Entities[_snapshot.Count++] = entity;
                }
            }
        }

        CompositeHandleFilter BuildSortFilter(CompositeHandleFilter baseFilter)
        {
            var sortFilter = new CompositeHandleFilter();
            if (baseFilter.IsValid) sortFilter.Merge(baseFilter);

            if (_sortComponentHandle.HasValue)
            {
                var handles = new List<ComponentsHandle>(1) { _sortComponentHandle.Value };
        ...
        bool HasSortHandle(World<TWorld>.Entity entity)
        {
            if (_sortComponentHandle.HasValue) return _sortComponentHandle.Value.Has(entity.ID);
            return false;
        }

        bool IsAlreadyInSnapshot(World<TWorld>.Entity entity, int sortedCount)
        {
