    ...
    public abstract class EcsDebug<TWorld> where TWorld : struct, IWorldType
    {
        ...
        public static void AddSystem<TSystemsType>() where TSystemsType : struct, ISystemsType
        {
            ...
            if (!World<TWorld>.Systems<TSystemsType>.IsInitialized)
                throw new InvalidOperationException(
                    "StaticEcsWorldDebug Debug mode connection is possible only when systems initialized");

            StaticEcsDebugData.Systems[typeof(TSystemsType)] = (World<TWorld>.Systems<TSystemsType>.AllSystems,
                World<TWorld>.Systems<TSystemsType>.AllSystemsCount, typeof(TWorld));
#endif
#endif
        }

        public static void AddWorld<TSystemsType>(int eventHistoryCount = 8192,
            Func<EntityGID, string> windowEntityNameFunction = null, short systemOrder = short.MaxValue)
            where TSystemsType : struct, ISystemsType
        {
            ...
            if (World<TWorld>.Status == WorldStatus.NotCreated)
                throw new InvalidOperationException(
                    "StaticEcsWorldDebug Debug mode connection is possible only after world creation");
        ...
        public static void RemoveWorld()
        {
            ...
            foreach (var key in toRemove) StaticEcsDebugData.Systems.Remove(key);
