...
namespace FFS.Libraries.StaticEcs.Unity
{
    ...
    internal class WorldData<TWorld> : AbstractWorldData, World<TWorld>.IEventsDebugEventListener
        where TWorld : struct, IWorldType
    {
        SpinLock _eventListenerLock = new(false);

        public void OnEventSent<T>(World<TWorld>.Event<T> value) where T : struct, IEvent
        {
            var typeIdx = TypeIdx.Create<T>();

            if (typeIdx.Type.IsIgnored()) return;

            var lockTaken = false;
            _eventListenerLock.Enter(ref lockTaken);

            if (EventsReceived.TryGetValue(typeIdx.Type, out var index)) index++;
        ...
        void OnEventDelete<T>(World<TWorld>.Event<T> value, EventStatus eventStatus) where T : struct, IEvent
        {
            if (TypeIdx.Create<T>().Type.IsIgnored()) return;
